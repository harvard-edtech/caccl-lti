...
var LaunchType_1 = __importDefault(require("./types/LaunchType"));
/*------------------------------------------------------------------------*/
/*                                 Helpers                                */
/*------------------------------------------------------------------------*/
/**
 ...
 */
var parseIntIfTruthy = function (val) {
    if (val !== undefined) {
 ...
 */
var splitIfTruthy = function (val) {
    if (val) {
 ...
 */
var parseLaunch = function (req) {
    /*----------------------------------------*/
    /*              Parse Launch              */
    /*----------------------------------------*/
    // Extract body
    var launchBody = req.body;
    // Detect launch type
    var wasAssignmentLaunch = (launchBody.custom_canvas_assignment_id
        && launchBody.custom_canvas_assignment_title
        && launchBody.custom_canvas_assignment_points_possible);
    var launchType = (wasAssignmentLaunch
    ...
    if (wasAssignmentLaunch) {
        ...
        // Parse outcomes
        var acceptedDataValues = (launchBody.ext_outcome_data_values_accepted.split(','));
        outcome = {
    ...
    var roles = String(launchBody.roles || '').split(',');
    // Parse custom parameters
    var customParams = new Map();
